  ...
  async componentDidMount() {
    for (let i = 0; i < this.state.states.length; i++) {
      try {
        ...
        const response = await axios.get(`https://brasil.io/api/dataset/covid19/caso/data?is_last=True&state=${this.state.states[i]}`, { timeout: 15000 });
        ...
        setTimeout(async () => {
          ...
          const response2 = await axios.get(`https://brasil.io/api/dataset/covid19/caso/data?is_last=True&state=${this.state.states[i]}`, { timeout: 15000 });
          await this.setState({ estado: this.state.states[i] });
          // await console.log(this.state.estado)
